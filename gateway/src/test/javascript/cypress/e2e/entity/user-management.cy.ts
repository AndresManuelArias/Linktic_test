...
describe('UserManagement e2e test', () => {
  ...
  let username: string;
  let password: string;
  const userManagementSample = { login: 'Victor72', email: 'Cornelius.Dare43@gmail.com' };
  ...
  describe('new UserManagement page', () => {
    ...
    it('should create an instance of UserManagement', () => {
      cy.get(`[data-cy="login"]`).type('Olive.Kihn63');
      cy.get(`[data-cy="login"]`).should('have.value', 'Olive.Kihn63');

      cy.get(`[data-cy="firstName"]`).type('Dusty');
      cy.get(`[data-cy="firstName"]`).should('have.value', 'Dusty');

      cy.get(`[data-cy="lastName"]`).type('Durgan');
      cy.get(`[data-cy="lastName"]`).should('have.value', 'Durgan');

      cy.get(`[data-cy="email"]`).type('Bradford40@gmail.com');
      cy.get(`[data-cy="email"]`).should('have.value', 'Bradford40@gmail.com');
